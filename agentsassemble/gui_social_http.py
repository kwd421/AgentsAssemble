...
def register_room_friend_profile_routes(
    ...
    *,
    ...
    def room_friends(ctx: RequestContext) -> None:
        ctx.send_json(room_friends_payload(ctx.deps.output_root, read_live_agents(ctx.deps.output_root)))
    ...
    def post_room_friend(ctx: RequestContext) -> None:
        ...
        ctx.send_json(
            {
                "friend": friend,
                **room_friends_payload(ctx.deps.output_root, read_live_agents(ctx.deps.output_root)),
            }
        )
    ...
    def delete_room_friend_route(ctx: RequestContext) -> None:
        try:
            deleted = delete_room_friend(ctx.deps.output_root, ctx.query_value("friend_id"))
        except ValueError as error:
            ctx.send_error(HTTPStatus.BAD_REQUEST, str(error))
            return
        ctx.send_json(
