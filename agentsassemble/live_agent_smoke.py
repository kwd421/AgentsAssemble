        ...
        with temp_dir_factory() as temp_dir:
            ...
            try:
                ...
                probe_event_ids, replies = _session_smoke_lobby_probe_replies(
                    ...
                    clean_meeting_id=clean_meeting_id,
                    expected_messages=expected_messages,
...
def _session_smoke_self_service_script(message: str) -> str:
    return "\n".join(
        [
            ...
            "        try:",
            "            timeout_payload = json.loads(wait.stdout or '{}')",
            "        except json.JSONDecodeError:",
            "            timeout_payload = {}",
            "        if isinstance(timeout_payload, dict) and timeout_payload.get('status') == 'timeout':",
            "            lobby_cursor = str(timeout_payload.get('last_observed_event_id') or '')",
            "            live_cursor = str(timeout_payload.get('last_observed_live_event_id') or '')",
            "            if lobby_cursor or live_cursor:",
            "                heartbeat('online', last_error='', last_observed_event_id=lobby_cursor, last_observed_live_event_id=live_cursor)",
            "        time.sleep(POLL)",
            "        continue",
            "    try:",
            ...
            "            )",
            "            reply = run_optional(insert_before_message_separator(reply_command, '--json'))",
            ...
            "    elif action == 'lobby' and source_event_id:",
            ...
            "        )",
            "        reply = run_optional(insert_before_message_separator(say_command, '--json'))",
            "        if reply is not None and reply.returncode == 0:",
            ...
            "        read_command = payload.get('read_command')",
            "        if not isinstance(read_command, list) or not read_command:",
            "            heartbeat('error', last_error='return packet read command missing', last_observed_live_event_id=source_event_id)",
            "            time.sleep(POLL)",
            "            continue",
            "        packet = run_optional([str(part) for part in read_command])",
            "        if packet is None or packet.returncode != 0:",
            "            heartbeat('error', last_error='return packet read failed', last_observed_live_event_id=source_event_id)",
            "            time.sleep(POLL)",
            "            continue",
            "        ack_command = payload.get('ack_command')",
            "        if isinstance(ack_command, list) and ack_command:",
            "            ack = run_optional([str(part) for part in ack_command])",
            "            if ack is None or ack.returncode != 0:",
            ...
            "    elif action == 'observe_lobby' and source_event_id:",
            "        ack_command = payload.get('ack_command')",
            "        if isinstance(ack_command, list) and ack_command:",
            "            ack = run_optional([str(part) for part in ack_command])",
            "            if ack is None or ack.returncode != 0:",
            "                heartbeat('error', last_error='lobby observation ack failed', last_observed_event_id=source_event_id)",
            "        else:",
            "            heartbeat('online', last_error='', last_observed_event_id=source_event_id)",
            "    time.sleep(POLL)",
        ]
    )
    ...
    for index in range(lobby_probe_count):
        probe_response = request_json(
            ...
            payload={
                ...
                "flow_meeting_id": clean_meeting_id,
            },
        )
        probe_event_id = _event_id(probe_response)
