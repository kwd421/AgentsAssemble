...
ROOM_FRIEND_TYPES = {"human", "subscription_ai", "api", "local", "unknown"}
...
def room_friend_type_for_agent(agent: dict[str, object]) -> str:
    ...
    if any(token in text for token in ("manual", "guest", "human", "remote_http_bridge")):
        return "human"
...
def _normalize_friend_record(payload: dict[str, Any]) -> dict[str, object]:
    ...
    handle = clean_lobby_text(payload.get("handle") or payload.get("agent_id"), limit=128)
...
def _friend_id_from_payload(payload: dict[str, object]) -> str:
    basis = clean_lobby_text(payload.get("agent_id") or payload.get("handle") or payload.get("display_name"), limit=96)
