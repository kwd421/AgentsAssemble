...
class ProviderCapabilityCatalog:
    """Best-effort native option discovery with a bounded process/version cache."""
    ...
    def payload(self, *, refresh: bool = False) -> list[dict[str, object]]:
        ...
        with self._lock:
            ...
            if not refresh and not self._cached:
                self._cached = self._fallback_payload()
                self._cached_at = now
                self._start_background_refresh_locked()
                return [dict(item) for item in self._cached]
        return self._refresh_payload()

    def _refresh_payload(self) -> list[dict[str, object]]:
    ...
    def _fallback_payload(self) -> list[dict[str, object]]:
            ...
            payload.append(
                {
                    **definition.public_payload(),
                    ...
                    "startable": bool(resolved),
                    "resolved_executable": str(resolved or ""),
                    "discovery_status": "fallback" if resolved else "unavailable",
                    "discovery_error": "" if resolved else "configured command missing",
                    "controls": _fallback_controls(definition.provider_id),
                }
            )
        ...
        payload.append(
            {
                ...
                "discovery_status": "fallback" if resolved else "unavailable",
                "discovery_error": "" if resolved else "configured command missing",
                "controls": _fallback_controls("opencode"),
            }
        )
        payload.append(_deepseek_payload())
        return payload
        ...
        base.update(
            {
                ...
                "startable": bool(resolved),
                "resolved_executable": str(resolved or ""),
                "discovery_status": "unavailable" if not resolved else "fallback",
                "controls": _fallback_controls(definition.provider_id),
            }
        )
        if not resolved:
    ...
    def _discover(self, provider_id: str, executable: str) -> list[dict[str, object]]:
        ...
        if provider_id == "claude":
            ...
            return _fallback_controls("claude") if code == 0 else []
        return []

    def _opencode_payload(self) -> dict[str, object]:
        resolved = self._resolver("opencode")
        controls = _fallback_controls("opencode")
        status = "unavailable" if not resolved else "fallback"
...
def _fallback_controls(provider_id: str) -> list[dict[str, object]]:
    if provider_id == "codex":
        return [
            _control("model", "모델", [_option("gpt-5.6-luna")], "gpt-5.6-luna", kind="combobox"),
            _control("reasoning_effort", "추론 강도", [_option(value) for value in ("low", "medium", "high", "xhigh", "max")], "low"),
            _control("service_tier", "응답 속도", [_option("default", "기본"), _option("priority", "Fast")], "default"),
            _permission_control(),
        ]
    if provider_id == "antigravity":
        return [_control("model", "모델", [_option("Gemini 3.5 Flash (Medium)")], "Gemini 3.5 Flash (Medium)"), _permission_control()]
    if provider_id == "grok":
        return [
            _control("model", "모델", [_option("grok-4.5"), _option("grok-composer-2.5-fast")], "grok-4.5"),
            _control("reasoning_effort", "추론 강도", [_option(value) for value in ("low", "medium", "high")], "medium"),
            _permission_control(),
        ]
    if provider_id == "claude":
    ...
    if provider_id == "opencode":
        return _opencode_controls(["opencode-go/glm-5.2"])
    return []
