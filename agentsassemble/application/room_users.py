"""Application-scoped stable user identity facade for room participants.
...
This module is application composition rather than room-domain policy: it owns
the process-global binding to the server-selected identity backend and the
legacy local-store fallback used by compatibility callers.
"""
