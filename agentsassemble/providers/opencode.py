...
class OpenCodeRuntime:
    ...
    def __init__(
        ...
        *,
        ...
        self.model = clean_room_text(model, limit=256) or "opencode-go/glm-5.2"
            ...
            for raw_line in event_response:
                ...
                if event_type == "permission.asked":
                    handle_provider_request(
                        lambda: handle_opencode_permission_request(
                            properties,
                            handler=self._provider_request_handler,
                            post_json=lambda path, payload: self._post_json(
                                path,
                                payload,
                ...
                if event_type == "question.asked":
                    handle_provider_request(
                        lambda: handle_opencode_question_request(
                            properties,
                            handler=self._provider_request_handler,
                            post_json=lambda path, payload: self._post_json(
                                path,
                ...
                if event_type == "message.updated":
                    ...
                    if str(info.get("role") or "") == "assistant" and message_id:
                        ...
                        observed_model = clean_room_text(
                        ...
                        if isinstance(info.get("error"), dict):
                            provider_error = opencode_error_message(info.get("error"))
    ...
    def _create_session(self) -> str:
        ...
        response.close()
        session_id = clean_room_text(result.get("id") if isinstance(result, dict) else "", limit=128)
        if not session_id:
