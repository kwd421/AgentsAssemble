...
class LiveCliRuntime:
    ...
    def __init__(
        ...
        *,
        ...
        self.input_mode = clean_room_text(input_mode, limit=64) or "line"
        ...
        self.profile_settings = {
            ...
            if clean_room_text(value, limit=256)
        }
    ...
    def deliver(self, events: list[dict[str, object]]) -> None:
        ...
        for event, line in zip(events, lines):
            ...
            event_id = clean_room_text(event.get("event_id"), limit=128)
            if event_id:
...
class ApiRuntime:
    ...
    def deliver(self, events: list[dict[str, object]]) -> None:
        if events:
            event_id = clean_room_text(events[-1].get("event_id"), limit=128)
            if event_id:
                self.last_seen_event_id = event_id
...
def _room_input_text(event: dict[str, object]) -> str:
    actor = clean_room_text(event.get("actor_id"), limit=128) or "unknown"
    content = clean_room_text(event.get("content"), limit=12000)
...
def _terminal_input_chunks(text: str, *, input_mode: str, submit_newline: str) -> tuple[bytes, ...]:
    ...
    if clean_room_text(input_mode, limit=64) == "bracketed_paste":
