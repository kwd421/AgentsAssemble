...
def user_for_credential(connection: Connection, auth_key: str) -> dict[str, object] | None:
    clean_key = clean_room_text(auth_key, limit=128)
    if not clean_key:
        return None
...
def resolve_credential_user(
    ...
    *,
    ...
    clean_display_name = clean_room_text(display_name, limit=64)
    clean_avatar = clean_room_text(avatar_image_url, limit=2048)
    clean_type = clean_room_text(participant_type, limit=32).lower()
...
def create_operator_pairing(
    ...
    *,
    ...
    clean_fingerprint = clean_room_text(token_fingerprint, limit=128)
    clean_room_id = clean_room_text(room_id, limit=128)
    clean_origin = clean_room_text(target_origin, limit=512)
    ...
    connection.execute(
        ...
        (
            ...
            clean_room_id,
            clean_origin,
            clean_room_text(created_at, limit=64),
            clean_room_text(expires_at, limit=64),
        ),
    )
    row = connection.execute(
...
def update_operator_pairing_redemption(
    ...
    *,
    ...
    clean_pairing_id = clean_room_text(pairing_id, limit=128)
    clean_auth_key = clean_room_text(auth_key, limit=128)
    clean_status = clean_room_text(status, limit=32)
    if (
    ...
    connection.execute(
        ...
        (
            clean_status,
            clean_room_text(completed_at, limit=64),
            clean_room_text(session_fingerprint, limit=128),
            clean_room_text(failure_code, limit=128),
            clean_pairing_id,
            clean_auth_key,
        ),
    )
