    ...
    def resolve(
        ...
        *,
        ...
        if invite.get("status") != "valid":
            reason = clean_room_text(invite.get("reason"), limit=64) or "invite_invalid"
            return {
        ...
        room_id = clean_room_text(invite.get("meeting_id"), limit=128)
        ...
        participant_id = clean_room_text(user.get("participant_id"), limit=128)
        ...
        participant_status = clean_room_text(participant.get("status"), limit=32).lower()
        membership_status = clean_room_text(membership.get("status"), limit=32).lower()
