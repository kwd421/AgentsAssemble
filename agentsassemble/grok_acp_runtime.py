...
class GrokAcpRuntime:
    ...
    def start(self) -> dict[str, object]:
        ...
        try:
            ...
            self._model = clean_lobby_text(model_state.get("currentModelId"), limit=128)
            capabilities = (
            ...
            if not session_id:
                ...
                session_id = clean_lobby_text(created.get("sessionId"), limit=128)
            if not session_id:
        ...
        *,
        ...
        try:
            while time.monotonic() < deadline:
                ...
                model = clean_lobby_text(result_meta.get("modelId"), limit=128)
                if model:
                    self._model = model
    ...
    def _resume_provider_session(self) -> str:
        ...
        try:
            ...
            self._provider_session_resume_failed = True
            self._provider_session_resume_error = clean_lobby_text(error, limit=1000)
            return ""
    ...
    def _reject_permission_request(self, message: dict[str, object]) -> None:
        request_id = message.get("id")
    ...
    def _provider_error_detail(self, fallback: str) -> str:
        ...
        while True:
            ...
            if time.monotonic() >= deadline:
                break
            time.sleep(0.01)
        return clean_lobby_text(fallback, limit=1000) or "Grok ACP request failed."
