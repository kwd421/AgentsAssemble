...
function SideChatMessage({ event }: { event: SideChatEvent }) {
  return (
    <div className="grid grid-cols-[40px_minmax(0,1fr)] gap-3 px-3 py-0.5 lg:px-4">
      <span className="mt-0.5 grid h-8 w-8 place-items-center justify-self-end rounded-full bg-panel-soft text-text-muted">
        <MessageSquare size={13} />
      </span>
      <div className="min-w-0 rounded-md border-l-2 border-idle/40 bg-panel-soft/40 px-3 py-1.5">
        <p className="flex flex-wrap items-baseline gap-2">
          <span className="text-[13px] font-semibold text-text-secondary preserve-words">
            {event.name || "사이드"}
          </span>
...
function SideChatComposer({
  ...
  async function handleSend() {
    ...
    setSendError("");
    try {
      const payload = await postSideChatMessage({ name: "나", side: "mine", message: trimmed });
  ...
  return (
    <div className="shrink-0 px-3 pb-3 lg:px-4">
      {(error || sendError) && (
        <p className="mb-2 rounded border border-danger/30 bg-danger/10 px-2.5 py-1.5 text-[12px] text-danger preserve-words">
          {sendError || "사이드챗 연결 대기 중"}
        </p>
      )}
      <div className="flex items-center gap-2 rounded-lg bg-panel-soft px-3 py-2">
        <input
          value={message}
          maxLength={2000}
        ...
        <button
          ...
          className="grid h-8 w-8 place-items-center rounded text-accent disabled:opacity-40"
          aria-label="사이드챗 보내기"
        >
          <Send size={17} />
        </button>
...
type FeedItem =
  ...
  | { kind: "side"; key: string; at: string; event: SideChatEvent };
...
export default function LiveView({
  ...
  // Official agent turns and unofficial side-chat share one scroll, but side-chat
  // stays visually distinct so Play Mode chatter never reads as official record.
  const feedItems = useMemo<FeedItem[]>(() => {
    const officialItems: FeedItem[] = events.map((event) => ({
      kind: "official",
      key: `o:${event.id}`,
      at: event.created_at || "",
      event,
    }));
    const sideItems: FeedItem[] = sideChatEvents.map((event) => ({
      kind: "side",
      key: `s:${event.id}`,
      at: event.created_at || "",
      event,
    }));
    return [...officialItems, ...sideItems].sort((left, right) => left.at.localeCompare(right.at));
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <ChannelHeader
        icon={<Radio size={20} />}
        title="실황"
        subtitle="에이전트 발언은 공식 흐름 · 내 메시지는 비공식 사이드챗"
        membersOpen={membersOpen}
        onToggleMembers={onToggleMembers}
      >
        {isRunning && (
        ...
        {feedItems.length === 0 ? (
          <div className="flex h-full flex-col items-center justify-center px-6 text-center">
            ...
            <p className="max-w-md text-[14px] text-text-secondary preserve-words">
              {isRunning
                ...
                : isFinished
                  ? "세션이 종료되었습니다. 로비에서 새 세션을 시작할 수 있습니다."
          ...
          feedItems.map((item) =>
            item.kind === "official" ? (
              ...
              <SideChatMessage key={item.key} event={item.event} />
            )
          )
        )}
      </div>

      <SideChatComposer onPosted={onSideChatPosted} error={sideChatError} />
    </div>
  );
}
