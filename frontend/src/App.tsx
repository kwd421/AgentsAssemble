              ...
              <section key={section.label} className="dc-channel-section">
                ...
                {channels.map(({ id, label, icon: Icon }) => (
                  <div key={id}>
                    ...
                    {id === "live" && (
                      <div className="dc-thread-list" aria-label="실황 하위 채팅">
                        <button
                          type="button"
                          onClick={() => goToChannel("live")}
                          className="dc-thread-row"
                        >
                          <Radio size={14} />
                          공식 흐름
                        </button>
      ...
      <main className="dc-chat flex min-w-0 flex-1 flex-col" aria-label="채널 내용">
        {channel === "friends" && !guestLocked ? (
          ...
          <LiveView
            ...
            sideChatEvents={activeRoomFlowVisible ? sideChatEvents : []}
            sideChatError={activeRoomFlowVisible ? sideChatError : null}
