...
class StaticUiAssetTests(unittest.TestCase):
    ...
    def test_react_discord_room_sidebar_uses_real_invite_and_context_actions(self):
        ...
        self.assertIn("읽기 전용 초대 링크를 로컬 DM으로 받습니다", invite_modal_source)
        self.assertIn("이 링크로 들어온 사람은 이 방만 보고 메시지는 보낼 수 없습니다.", invite_modal_source)
        self.assertIn("또는 친구에게 읽기 전용 초대 링크 전송하기", invite_modal_source)
        self.assertIn("읽기 전용 링크로 들어온 사람은 방을 보기만 합니다.", invite_modal_source)
        ...
        self.assertIn("if (!readOnlyInvite)", app_source)
    ...
    def test_react_public_join_route_renders_guest_only_invite_panel(self):
        ...
        self.assertIn('query.get("scope")', room_dock_model_source)
        ...
        self.assertIn("roomSessionToken={guestSession?.sessionToken || \"\"}", app_source)
        ...
        self.assertIn("roomSessionToken ? postRoomSay", lobby_composer_source)
