...
class StaticUiAssetTests(unittest.TestCase):
    ...
    def test_react_discord_home_friends_uses_persisted_room_friends(self):
        ...
        self.assertIn('type Channel = "home" | DiscordRoomChannelId;', app_source)
        self.assertIn("import DiscordRoomSidebar", app_source)
    ...
    def test_react_discord_room_sidebar_uses_real_invite_and_context_actions(self):
        app_source = frontend_file("App.tsx")
        sidebar_source = frontend_file("views/components/DiscordRoomSidebar.tsx")

        self.assertIn('type Channel = "home" | DiscordRoomChannelId;', app_source)
        self.assertIn("type DiscordRoomChannelId", app_source)
        self.assertIn("const CHANNELS: DiscordRoomChannel[]", app_source)
        self.assertIn('description: "준비, 초대, 시작 전 대화"', app_source)
        self.assertIn('className="ops-nav-scroll order-3 flex', app_source)
        ...
        self.assertIn("export type DiscordRoomChannelId", sidebar_source)
        self.assertIn("import RoomInvitePanel", sidebar_source)
        self.assertIn('aria-label="서버 목록"', sidebar_source)
        self.assertIn('aria-label="방 채널"', sidebar_source)
        self.assertIn("Text Channels", sidebar_source)
        self.assertIn('role="menu"', sidebar_source)
        self.assertIn('role="menuitem"', sidebar_source)
        self.assertIn("읽음으로 표시하기", sidebar_source)
        self.assertIn("서버에 초대하기", sidebar_source)
        self.assertIn("서버 나가기", sidebar_source)
        self.assertIn("setLastReadAt", sidebar_source)
        self.assertIn("setInviteOpen(true)", sidebar_source)
        self.assertIn("onLeaveRoom();", sidebar_source)
        self.assertIn("<RoomInvitePanel meetingId={meetingId} />", sidebar_source)
