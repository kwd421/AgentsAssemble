...
class PackageMapTests(unittest.TestCase):
    ...
    def test_room_user_facade_uses_application_ownership(self) -> None:
        graph = load_package_graph(ROOT)
        package_map = build_package_map(ROOT)

        self.assertEqual(
            graph.domains["agentsassemble.application.room_users"],
            "application",
        )
        owned_line = next(
            line
            for line in package_map.splitlines()
            if line.startswith("| `agentsassemble.application.room_users` |")
        )
        compatibility_line = next(
            line
            for line in package_map.splitlines()
            if line.startswith("| `agentsassemble.room_users` |")
        )
    ...
    def test_stable_entry_uses_application_ownership(self) -> None:
        graph = load_package_graph(ROOT)
        package_map = build_package_map(ROOT)

        self.assertEqual(
            graph.domains["agentsassemble.application.stable_entry"],
            "application",
        )
        owned_line = next(
            line
            for line in package_map.splitlines()
            if line.startswith("| `agentsassemble.application.stable_entry` |")
        )
        compatibility_line = next(
            line
            for line in package_map.splitlines()
            if line.startswith("| `agentsassemble.stable_entry` |")
        )
        self.assertTrue(owned_line.endswith("| in-target-package |"))
        self.assertIn("| compatibility |", compatibility_line)
        self.assertTrue(compatibility_line.endswith("| compatibility-shim |"))

    def test_public_invite_runtime_uses_application_ownership(self) -> None:
        graph = load_package_graph(ROOT)
        package_map = build_package_map(ROOT)

        self.assertEqual(
            graph.domains["agentsassemble.application.public_invite_runtime"],
            "application",
        )
        owned_line = next(
            line
            for line in package_map.splitlines()
            if line.startswith(
                "| `agentsassemble.application.public_invite_runtime` |"
            )
        )
        compatibility_line = next(
            line
            for line in package_map.splitlines()
            if line.startswith("| `agentsassemble.public_invite_runtime` |")
        )
        self.assertTrue(owned_line.endswith("| in-target-package |"))
        self.assertIn("| compatibility |", compatibility_line)
        self.assertTrue(compatibility_line.endswith("| compatibility-shim |"))
    ...
    def test_websocket_codec_uses_web_transport_ownership(self) -> None:
        graph = load_package_graph(ROOT)
        package_map = build_package_map(ROOT)

        self.assertEqual(
            graph.domains["agentsassemble.web.websocket_codec"],
            "web",
        )
        owned_line = next(
            line
            for line in package_map.splitlines()
            if line.startswith("| `agentsassemble.web.websocket_codec` |")
        )
        compatibility_line = next(
            line
            for line in package_map.splitlines()
            if line.startswith("| `agentsassemble.room_websocket` |")
        )
        self.assertTrue(owned_line.endswith("| in-target-package |"))
        self.assertIn("| compatibility |", compatibility_line)
        self.assertTrue(compatibility_line.endswith("| compatibility-shim |"))

    def test_room_client_uses_web_transport_ownership(self) -> None:
        graph = load_package_graph(ROOT)
        package_map = build_package_map(ROOT)

        self.assertEqual(graph.domains["agentsassemble.web.room_client"], "web")
        owned_line = next(
            line
            for line in package_map.splitlines()
            if line.startswith("| `agentsassemble.web.room_client` |")
        )
        compatibility_line = next(
            line
            for line in package_map.splitlines()
            if line.startswith("| `agentsassemble.ws_room_client` |")
        )
        self.assertTrue(owned_line.endswith("| in-target-package |"))
        self.assertIn("| compatibility |", compatibility_line)
        self.assertTrue(compatibility_line.endswith("| compatibility-shim |"))
    ...
    def test_nested_domain_modules_use_their_package_owner(self) -> None:
        ...
        for compatibility_module in (
            ...
            "agentsassemble.room_turn_coordinator",
    ...
    def test_cleanup_diagnostics_use_the_owned_package(self) -> None:
        graph = load_package_graph(ROOT)
        package_map = build_package_map(ROOT)

        self.assertEqual(
            graph.domains["agentsassemble.diagnostics.cleanup"],
            "diagnostics",
        )
        owned_line = next(
            line
            for line in package_map.splitlines()
            if line.startswith("| `agentsassemble.diagnostics.cleanup` |")
        )
        compatibility_line = next(
            line
            for line in package_map.splitlines()
            if line.startswith("| `agentsassemble.cleanup_report` |")
        )
        self.assertTrue(owned_line.endswith("| in-target-package |"))
        self.assertIn("| compatibility |", compatibility_line)
        self.assertTrue(compatibility_line.endswith("| compatibility-shim |"))
