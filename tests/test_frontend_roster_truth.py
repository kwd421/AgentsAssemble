...
class FrontendRosterTruthTests(unittest.TestCase):
    ...
    def test_active_agent_session_room_events_feed_live_view_without_flow(self):
        ...
        room_connection = frontend_file("views/components/RoomConnectionPanel.tsx")
        projection = frontend_file("lib/roomEventProjection.ts")
        ...
        self.assertIn("projectRoomEventsToTimeline", app)
        self.assertIn("projectRoomEventProgress", app)
        self.assertIn("export function projectRoomEventsToTimeline", projection)
