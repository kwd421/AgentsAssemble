...
class LiveAgentSmokeTests(unittest.TestCase):
    ...
    def test_session_smoke_runs_start_reply_check_resume_restart_and_stop_sequence(self):
        ...
        def request_json(url, *, method="GET", payload=None, timeout_seconds=None):
            ...
            if url.endswith("/api/live-agent-sessions/start"):
                ...
                self.assertIn("AGENTSASSEMBLE_HEARTBEAT_COMMAND_TEMPLATE", self_service_command)
                self.assertIn("shlex.split", self_service_command)
                self.assertIn("replace_tokens", self_service_command)
                self.assertIn("timeout_payload", self_service_command)
                self.assertIn("last_observed_event_id", self_service_command)
                self.assertIn("last_observed_live_event_id", self_service_command)
                self.assertNotIn("current_engagement_mode() == 'always'", self_service_command)
                ...
                while time.monotonic() < deadline:
                    if calls_path.exists():
                        ...
                        if say_call:
                            ...
                            heartbeat_after_say = any(
                                call and call[0] == "heartbeat" and index > say_index
                                for index, call in enumerate(calls)
                            )
                        else:
                            heartbeat_after_say = False
                        if say_call and heartbeat_after_say:
                            break
                    time.sleep(0.05)
                if say_call is None:
                    stderr = process.stderr.read() if process.poll() is not None and process.stderr is not None else ""
    ...
    def test_session_smoke_self_service_script_reads_return_packet_before_ack_without_replying(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            fake_cli = root / "fake_cli.py"
            calls_path = root / "calls.jsonl"
            ...
            try:
                ...
                while time.monotonic() < deadline:
                    if calls_path.exists():
                        ...
                        heartbeat_call = next((call for call in calls if call and call[0] == "heartbeat"), None)
                        if heartbeat_call:
                            break
                    time.sleep(0.05)
                if heartbeat_call is None:
                    stderr = process.stderr.read() if process.poll() is not None and process.stderr is not None else ""
                    self.fail(f"self_service script did not ack return packet; stderr={stderr}")
            finally:
                if process.poll() is None:
                    process.terminate()
                    try:
    ...
    def test_session_smoke_self_service_script_reports_error_when_return_packet_read_launch_fails(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            fake_cli = root / "fake_cli.py"
            calls_path = root / "calls.jsonl"
            ...
            try:
                ...
                if error_heartbeat is None:
                    stderr = process.stderr.read() if process.poll() is not None and process.stderr is not None else ""
                    self.fail(f"self_service script did not report return packet read launch failure; stderr={stderr}")
            finally:
                if process.poll() is None:
                    process.terminate()
                    try:
            ...
            online_acks = [
                ...
                for call in calls
                if call
                and call[0] == "heartbeat"
                and call[call.index("--status") + 1] == "online"
                and "--last-observed-live-event-id=packet-1" in call
            ]
    ...
    def test_session_smoke_self_service_script_keeps_replying_when_heartbeat_template_is_missing(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                if say_call is None:
                    stderr = process.stderr.read() if process.poll() is not None and process.stderr is not None else ""
                    self.fail(f"self_service script did not post after heartbeat failure; stderr={stderr}")
            finally:
                if process.poll() is None:
                    process.terminate()
                    try:
            ...
            self.assertEqual(say_call[0], "say")

    def test_session_smoke_self_service_script_reports_error_when_lobby_reply_fails(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            fake_cli = root / "fake_cli.py"
            calls_path = root / "calls.jsonl"
            ...
            try:
                ...
                if error_heartbeat is None:
                    stderr = process.stderr.read() if process.poll() is not None and process.stderr is not None else ""
                    self.fail(f"self_service script did not report reply failure; stderr={stderr}")
            finally:
                if process.poll() is None:
                    process.terminate()
                    try:
