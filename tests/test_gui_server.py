...
class GuiServerTests(unittest.TestCase):
    ...
    def test_live_agent_join_brief_http_endpoint_returns_safe_packet_without_registering(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("mcp", payload["mcp"]["command"])
            self.assertEqual(payload["mcp"]["command"][payload["mcp"]["command"].index("--server") + 1], server_url)
            self.assertEqual(payload["mcp"]["command"][payload["mcp"]["command"].index("--agent-id") + 1], "external-reviewer")
