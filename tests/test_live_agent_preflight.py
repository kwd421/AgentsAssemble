...
class LiveAgentPreflightTests(unittest.TestCase):
    ...
    def test_preflight_rejects_non_string_command_entries(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            config_path = Path(temp_dir) / "live-agents.json"
            config_path.write_text(
                json.dumps({"agents": [{"agent_id": "agent-a", "command": ["python3", None]}]}),
                encoding="utf-8",
            )
            ...
            self.assertEqual(report["summary"], {"agents": 0, "failed_agents": 0, "checks_failed": 1})
            self.assertEqual(report["checks"][0]["id"], "config_load")
            self.assertEqual(report["checks"][0]["message"], "Live agent command entries must be strings.")

    def test_preflight_rejects_non_finite_timing_values(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            config_path = Path(temp_dir) / "live-agents.json"
            config_path.write_text(
                json.dumps(
