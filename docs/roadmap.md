...
- The GUI Lobby can start or resume a resident live-agent session, optionally run bounded remaining template rounds after the session is ready, recover historical resident process groups, call one moderator-controlled official round, or run bounded remaining template rounds from the same `상주 실행` panel, using the real live-agent official-turn paths and reporting sanitized reply counts.
