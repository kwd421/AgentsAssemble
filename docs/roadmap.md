...
- Resident health and session readiness now treat connection evidence as binding-compatible rather than id-only: a same-id roster row with the wrong provider or connection kind degrades readiness with safe mismatch attention instead of being counted as a connected resident.
