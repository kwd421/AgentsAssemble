...
- The React/Vite Discord-style room client is the single served GUI frontend.
  `/` serves the built React client, `/app/` is the same React alias, and retired
  vanilla frontend paths do not serve the old console. `frontend-info` reports
  the room-client URL and whether the ignored `frontend/dist` build output is
  present.
