...
- The GUI Lobby can start a resident live-agent session and then call a moderator-controlled official round from the same `상주 실행` panel, using the real `/api/meetings/<meeting_id>/live-agent-turns/round` path and reporting sanitized reply counts.
