...
- The React/Vite frontend is the Python GUI's default room client when
  `frontend/dist` exists. If the build is missing, `/` and `/app/` report that
  the React build is required; legacy static routes are retired. `frontend-info`
  reports the React URL and build status.
...
- No-Tailscale multi-host now has a design document, a LAN invite token PoC, and a working web invite v1 with GUI-assisted public tunnel setup. `live-agent lan-invite create/verify` signs and verifies an HMAC-SHA256 admission packet, while `/api/public-invite/*`, `/api/room-invite/create`, `/join`, `/api/room/say`, and `/leave` provide host-gated browser participation with in-memory session tokens and server-enforced identity. This does not start provider CLIs, provide durable auth/revocation beyond server-memory leave/expiry, solve NAT traversal, or make relay/WebRTC ready; public internet access still depends on an operator-supplied tunnel.
