...
- Resident finalization now announces per-agent return packets as targeted non-official room events. Terminal, self-service, and external/manual agents can receive `action: "return_packet"` from `wait-next`, read the relative packet paths, and acknowledge the live cursor without posting a lobby reply.
