...
- The GUI live-agent form can generate the same safe external `join-brief` packet as the CLI/API, so an operator can hand another AI a register/wait-next loop without registering it, starting a provider, writing files, or granting broad automatic entry.
