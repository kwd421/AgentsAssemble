...
- No-Tailscale multi-host now has a design document, a LAN invite token PoC, and a working web invite v1. `live-agent lan-invite create/verify` signs and verifies an HMAC-SHA256 admission packet, and `/api/room-invite/create`, `/join`, `/api/room/say`, `/leave` provide authenticated room participation with in-memory session tokens and server-enforced identity. This does not start provider CLIs, provide durable auth/revocation beyond server-memory leave/expiry, solve NAT traversal, or make relay/WebRTC ready. Host-only invite creation is not enforceable without server-side auth.
