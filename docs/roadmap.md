...
- The GUI live-agent form can generate the same safe external `join-brief` packet as the CLI/API, so an operator can hand another AI a register/wait-next/leave loop without registering it, starting a provider, writing files, or granting broad automatic entry. External/manual agents now also have `live-agent leave`, a safe offline heartbeat command that clears stale error text, records a safe `live_agent.leave` operation, and does not delete the roster row or stop supervised groups.
