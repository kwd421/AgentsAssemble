...
- Resume-capable real providers now have a narrower direct continuity proof: `live-agent continuity-proof` requires current `--approve-real-providers`, calls the Codex or Kiro resident runner for two turns, verifies provider-owned suffix recall without replaying the private continuity code, and reports only safe booleans, lengths, provider kind, and a short session-id suffix. This complements `real-session-smoke`; it does not prove room admission, cleanup, official-turn quality, or restart behavior.
