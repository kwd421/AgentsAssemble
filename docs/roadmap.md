...
- The GUI live-agent form can generate the same safe external `join-brief` packet as the CLI/API, so an operator can hand another AI a register/wait-next/leave loop without registering it, starting a provider, writing files, or granting broad automatic entry. External/manual agents now also have safe `live_agent.register` and `live_agent.leave` operation records around explicit entry and exit; `live-agent leave` clears stale error text without deleting the roster row or stopping supervised groups.
