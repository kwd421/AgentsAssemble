...
| `grok_build_cli` | Planned local CLI/provider candidate. Local `grok` contract discovery found version `grok 0.2.2 (c9b7cdec23a)`, `--single`, prompt file/JSON input, `--resume`, `--continue`, output formats, sandbox flags, session listing, and `grok agent` modes including `stdio`, `headless`, `serve`, and `leader`. An approved isolated-git-cwd JSON stdout probe proved two-turn suffix recall through explicit `--resume <sessionId>`. | Future provider-specific `grok_live_session`/runner is justified by evidence; `self_service` and `terminal_session` remain possible later paths; `local_cli` remains stateless delegate only. | `grok_session_resume`; provider-managed session history is proven for the narrow JSON stdout `text` field. Full process output can echo prompt material in stderr, so raw stdout/stderr must not be persisted. This still does not prove room admission, official-turn quality, tool safety, stop/restart behavior, or sandboxing. | `advisory` until a sandboxed launcher exists. | Grok-specific runner that parses JSON stdout `text`, captures only safe session suffix evidence, and redacts stderr/logs. | Add a fake Grok runner regression, then a real Grok room smoke with explicit approval before marking resident-ready. |
