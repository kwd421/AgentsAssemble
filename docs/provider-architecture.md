...
`codex_live_session` is the first live-session slice. Meeting turns use Codex CLI session ids and `codex exec resume` so repeated turns can continue the same Codex session history. Resident live-agent configs use `provider_kind: "codex_live_session"` with `connection_kind: "live_session"`; the resident runner calls Codex CLI through `codex exec` / `codex exec resume` rather than through the JSONL fake-session protocol. Meeting policy still marks this participant read-only, but the current resident Codex launch path does not force a Codex sandbox or approval policy, so filesystem, git, push, release, and credential boundaries remain advisory until a constrained launch path is implemented and verified. This is not native Codex/Claude channel injection or verified PTY persistence.
