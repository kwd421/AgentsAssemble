...
`restart-session --run-remaining-rounds` uses the same ready gate as `start-session` and `resume-session`: remaining official rounds run only when the restarted session returns `status: "ready"`. Otherwise `auto_rounds.status` is `skipped` with `reason: "session_not_ready"` and no official turn request events are appended. The round options are the same `--round-timeout`, `--max-rounds`, and `--stop-on-timeout` fields used by start/resume.
...
`recover-session --run-remaining-rounds` uses the same ready gate as the other session entrypoints: remaining official rounds run only when the recovered session returns `status: "ready"`. Otherwise `auto_rounds.status` is `skipped` with `reason: "session_not_ready"` and no official turn request events are appended. The round options are the same `--round-timeout`, `--max-rounds`, and `--stop-on-timeout` fields used by start/resume/restart.
