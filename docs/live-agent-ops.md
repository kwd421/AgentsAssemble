...
`/` serves the React Discord-style room client once
`npm --prefix frontend run build` exists. There is no dependency-light vanilla
fallback anymore: `/legacy/`, `/legacy/static/*`, and `/static/*` are retired
frontend paths, and `/app/` is the same React client alias. The React/Vite dev
server in `frontend/` runs on port 5173 and proxies API calls to this backend by
default. Use
`python3 -m agentsassemble.cli frontend-info` for read-only launch guidance;
that command prints the React room-client URL plus `app_build_status` and
`app_referenced_assets_present` so partial or stale builds explain why the room
client is unavailable. It does not start a dev server, GUI backend, or provider
CLI. The React-only integration is tracked in
...
The current frontend is the React Discord-style room client. Natural-language
