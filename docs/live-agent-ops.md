...
The command first reads `GET /api/live-agent-sessions/readiness` when both `meeting_id` and `group_id` are known. If that snapshot is already `ready`, it prints the session summary and performs no control-plane POST unless `--probe-bound-agents` or `--run-remaining-rounds` is requested. With either post-ready check requested, the ready no-op path posts to `/api/live-agent-sessions/ensure` so the server can run the same bounded probe and remaining-round gates as the GUI `세션보장` control without starting, resuming, restarting, or recovering the process group. If the meeting is missing or no target ids are supplied, it calls `start-session`. If the target meeting exists but the process group is absent, or if the target is degraded with a `running` or `restarting` process, it calls `resume-session` so the existing meeting can reconnect without recreating the meeting or forcing an unnecessary restart. If the process is `stopped`, it calls `restart-session`; if it is `unknown` or `error` for an existing process group, it calls `recover-session`. The CLI and API share this same `session_ensure_action` policy helper, so the action chosen for a readiness snapshot does not drift between `ensure-session` and `세션보장`. After any mutating command, `ensure-session` uses the same read-only readiness wait as `--wait-ready`, so the final success decision comes from the targeted readiness snapshot rather than from the mutating command's shorter response, while preserving any `reply_probe` and `auto_rounds` evidence returned by the control-plane call. It returns `0` once ready and `1` with the last observed summary on timeout.
...
Use the GUI restart button on a stopped, crashed, or recovered group to relaunch it from the persisted `config_path` and `server`. Restart and recovery refuse a blank persisted `config_path` before preflight or process launch, so corrupted historical rows fail with a clear missing-launch-config error instead of treating the current directory as a resident config. Restart also reruns preflight before launching, so a config or environment that became invalid while the group was down is refused synchronously. The HTTP restart path is:
