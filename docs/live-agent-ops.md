...
For meeting-bound agents, the room snapshot also includes compact shared_memory in `/api/live-agents/<agent_id>/room`. It is derived from the official-only `shared_memory/index.json` or the same in-memory meeting state and includes bounded counts, rolling summary, decisions, open questions, and action items. `wait-room-event`, `wait-official-turn`, and wait-next event payloads include `room.shared_memory` from that same compact room snapshot so terminal, self-service, and external/manual agents see the same official-only background context as parent-managed resident prompts. It does not include lobby chat, side chat, review checkpoints, private turn requests for other agents, provider prompts, config paths, endpoint URLs, auth refs, log tails, or provider output.
...
`scripts/my_self_service_agent.py` is the checked-in custom wrapper example used by `configs/live-agents.self-service.example.json`. It reads the exported command templates, polls `wait-next`, posts lobby replies through `say`, posts official replies through `official-reply`, acknowledges `return_packet` actions through the returned `ack_command` or heartbeat cursor fallback, and sends `working`, `online`, or `error` heartbeats without reading stdin or receiving per-event prompt injection from the parent supervisor.
