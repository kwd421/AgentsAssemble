...
- The default CLI process list and GUI process rows show the latest lifecycle offline summary beside the last event label, such as `last event restart_scheduled, offline 1/2, wrong_meeting agent-b`, so crash-time roster reconciliation evidence is visible without opening JSON. The CLI process list also shows the latest sanitized watchdog reason from recent lifecycle history, such as `last reason stale_watchdog missing manifest agent agent-a`, when the latest row event did not carry that reason itself.
