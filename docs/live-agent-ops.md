...
For meeting-bound agents, the room snapshot also includes compact shared_memory in `/api/live-agents/<agent_id>/room`. It is derived from the official-only `shared_memory/index.json` or the same in-memory meeting state and includes bounded counts, rolling summary, decisions, open questions, and action items. It does not include lobby chat, side chat, review checkpoints, private turn requests for other agents, provider prompts, config paths, endpoint URLs, auth refs, log tails, or provider output.
