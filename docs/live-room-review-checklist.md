...
- Latest implementation commit: `c1f92b8 Move owner avatars inside lobby bubbles`
...
Follow-up GUI review showed the remaining lobby clipping affected the right-side owner avatar. Commit `c1f92b8 Move owner avatars inside lobby bubbles` keeps the owner avatar visible but moves local user/local-agent avatars into the bubble header, removing the separate right-edge circle that was being clipped by the panel edge or scrollbar.
