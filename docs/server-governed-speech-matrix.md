...
2. Add `governed_official_reply()` separately for official-turn replies.
3. Decide side-chat, DM, and `/api/lobby/remote` policy: operator-only trusted
   routes or participant-governed speech routes. Do not leave them ambiguous.
4. Add flood/rate and chain-depth policy at the shared service boundary.
5. Add a focused governance test suite that attacks every speech entry with:
