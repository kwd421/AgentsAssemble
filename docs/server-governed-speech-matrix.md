...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `POST /api/room/say` | `ENFORCED` by session token via `RequestContext.require_posting_session` in `agentsassemble/web/router.py:351`; identity is normalized in `agentsassemble/gui_room_http.py:196` and stamped server-side in `agentsassemble/room_speech.py:146`. | `ENFORCED` read-only reject in `agentsassemble/web/router.py:356`; shared policy check in `agentsassemble/room_speech.py:111`; HTTP preserves the pre-body mute check order in `agentsassemble/gui_room_http.py:197`. | `DELEGATED` to governed lobby payload stamping in `agentsassemble/room_speech.py:146` and event cleanup in `agentsassemble/meeting_events.py:97`. | `ENFORCED` server chain-depth and burst limits in `agentsassemble/room_speech.py:71`; HTTP maps chain/rate rejection in `agentsassemble/gui_room_http.py:85`. | `N/A` for ordinary lobby speech; flow turn CAS only exists on live-agent lobby replies. | Appends lobby event through `governed_lobby_say` in `agentsassemble/gui_room_http.py:215`. | Complete: HTTP room say uses the shared governed lobby-say service while keeping transport parsing/error behavior in the route. |
| WebSocket `op: "say"` | `ENFORCED` by the verified WS identity; client metadata is safelisted in `agentsassemble/ws_room_session.py:33`, then real server deps normalize/stamp through `governed_lobby_say` in `agentsassemble/gui.py:8310` and `agentsassemble/room_speech.py:146`. | `ENFORCED` read-only and mute checks still fast-fail in `agentsassemble/ws_room_session.py:188`; real server deps re-check shared policy in `agentsassemble/room_speech.py:111`. | `DELEGATED` through WS metadata safelist in `agentsassemble/ws_room_session.py:196` and governed lobby append in `agentsassemble/gui.py:8310`. | `ENFORCED` server chain-depth and burst limits in `agentsassemble/room_speech.py:71`; WS converts rejections to protocol errors in `agentsassemble/gui.py:8319`. | `N/A` for ordinary lobby speech. | Appends lobby event through `governed_lobby_say` in `agentsassemble/gui.py:8310`. | Complete: WS shares the same server-side identity, chain/rate, and append service as HTTP, though protocol-local burst/dedup is still not separate. |
| `POST /api/room/channel-say` | `ENFORCED` for session callers by `ctx.session()`/host fallback in `agentsassemble/gui_room_http.py:537`; session identity is normalized through `ActorIdentity.from_mapping` in `agentsassemble/gui_room_http.py:543`; local operator identity is explicitly built in `agentsassemble/gui_room_http.py:573`; final identity stamping is shared in `agentsassemble/room_speech.py:179`. | `ENFORCED` read-only for session callers in `_channel_caller`; shared session mute/read-only policy runs in `agentsassemble/gui_room_http.py:557`; host/local operator mute is `N/A/TRUSTED`. Channel existence/type is checked in `agentsassemble/gui_room_http.py:581`. | `DELEGATED` through `governed_channel_say` in `agentsassemble/room_speech.py:163` and event cleanup in `agentsassemble/meeting_events.py:399`. | `ENFORCED` server chain-depth and burst limits in `agentsassemble/room_speech.py:186`; HTTP maps chain/rate rejection in `agentsassemble/gui_room_http.py:600`. | `N/A` for custom channel speech today. | Appends channel event through `governed_channel_say` in `agentsassemble/gui_room_http.py:590`. | Complete: channel say uses the shared identity/sanitizer/rate core and a sibling governed channel append path, while channel validation stays route-local. |
| `POST /api/live-agents/{agent_id}/lobby` | `ENFORCED` from persisted live-agent state via heartbeat lookup in `agentsassemble/gui.py:3503`; actor id comes from the stored agent in `agentsassemble/gui.py:3507`; `ActorIdentity` is built in `agentsassemble/gui.py:3509`; final identity stamping is shared in `agentsassemble/room_speech.py:146`. Route dispatch starts in `agentsassemble/gui.py:11124`. | `ENFORCED` through shared mute/read-only policy in `agentsassemble/gui.py:3516`; read-only is `N/A` because this is a server-owned resident endpoint, not a guest invite session. | `DELEGATED` through `governed_lobby_say` in `agentsassemble/gui.py:3573`, shared payload stamping in `agentsassemble/room_speech.py:146`, and event cleanup in `agentsassemble/meeting_events.py:97`. | `ENFORCED` server chain-depth and burst limits in `agentsassemble/room_speech.py:71`; live-agent flow turn CAS remains separate. | `ENFORCED` for flow speaking actions through `_flow_turn_conflict` in `agentsassemble/gui.py:3550`, with duplicate/stale-turn checks in `agentsassemble/gui.py:3523`. | Appends lobby event through `governed_lobby_say` in `agentsassemble/gui.py:3573` and updates heartbeat after reply. | Complete: live-agent lobby replies share the lobby speech service while preserving live-agent endpoint marking, flow metadata, idempotency, and turn-CAS. |
...
4. Shared participant speech now has a small server-side chain-depth and burst
   guard, but operator/private DM cost bounds are still intentionally separate.
...
2. Add explicit operator/private DM rate or cost bounds if long-running rooms
   make those paths active automation surfaces.
3. Add a broader governance test matrix that attacks every speech entry with:
