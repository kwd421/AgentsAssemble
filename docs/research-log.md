...
- Implemented slice:
  ...
  - Added opt-in API credential model-list probes (`probe_mode: api`) for Anthropic, Gemini, and Grok providers. The probe reads `auth_ref` only in that explicit mode, calls only official HTTPS model-list endpoints with provider-specific auth headers, rejects custom hosts or URL secret-bearing components before network access, does not follow redirects or proxies, sends no prompt or meeting payload, and reports only safe model-list reachability evidence.
  - Added resident remote bridge live-agent runners. `live-agent run` and `run-group` can now use `connection_kind: remote_bridge` with an explicit endpoint and auth_ref, allowing the normal resident polling loop to send eligible lobby context to `/agentsassemble/run` and post the returned reply with actor, source event, and chain-depth metadata. This is separate from health probing and intentionally calls the bridge execution endpoint only when the operator starts that resident agent.
