...
| `features/social/friends.py`, `features/social/routes.py` | Active optional | Saved friend records, live-agent status projection, local profile, and friend DM routes; root compatibility exports remain in `room_friends.py` and `gui_social_http.py` | `tests/test_room_friends.py`, `tests/test_gui_server_social_http.py`, `tests/test_room_social_flows.py` |
