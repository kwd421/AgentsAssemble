...
| --- | --- | --- | --- |
| `legacy/gui_application.py` | Compatibility composition | Server-lifetime construction and registration bundle for retained meeting/live-agent routes; current `gui.py` injects patch seams and server-owned services; root module is a compatibility export | `tests/test_gui_application_services.py`, route-family tests below |
...
| `legacy/meeting/http/lobby.py` | Compatibility | HTTP lobby write/SSE plus explicit promotion and governed remote-bridge commands; command policy lives in `legacy/meeting/lobby_commands.py`; root HTTP module is a compatibility export | `tests/test_gui_server_streams_http.py`, `tests/test_gui_server_lobby_social.py`, `tests/test_lobby_promotion.py` |
| `legacy/meeting/http/meeting.py` | Compatibility | Legacy meeting list/detail, lifecycle, workroom queue, and meeting SSE transport; root HTTP module is a compatibility export | `tests/test_gui_server_meeting_payload.py`, `tests/test_gui_server_discovery_workroom.py`, `tests/test_gui_server_streams_http.py` |
| `legacy/meeting/http/lifecycle.py` | Compatibility | Retained meeting start/finalize HTTP commands; domain execution and operation audit live in `legacy/meeting/lifecycle.py`, with bounded audit projection in `legacy/meeting/operation_projection.py`; root HTTP module is a compatibility export | `tests/test_gui_legacy_meeting_lifecycle_http.py`, `tests/test_gui_server_session_lifecycle.py`, `tests/test_gui_server_moderation_finalization.py` |
| `legacy/meeting/http/review_checkpoint.py` | Compatibility | Resident review-checkpoint request, sequential reply wait, non-official artifact creation, and prompt-free operation audit; orchestration lives in `legacy/meeting/review_checkpoint.py`; root HTTP module is a compatibility export | `tests/test_gui_legacy_review_checkpoint_http.py`, `tests/test_legacy_review_checkpoint.py`, `tests/test_gui_server_moderation_finalization.py`, `tests/test_live_agent_review_checkpoints.py` |
| `legacy/meeting/http/official_turn.py` | Compatibility | Official-turn request, verified-reply call, and ordered sequence commands; execution/audit live in `legacy/meeting/official_turns.py` and per-meeting locking in `legacy/meeting/turn_scheduler.py`; root HTTP module is a compatibility export | `tests/test_gui_legacy_official_turn_http.py`, `tests/test_legacy_official_turns.py`, `tests/test_gui_server_turns.py` |
| `legacy/meeting/http/official_round.py` | Compatibility | Official round, remaining-round batch, preset expansion, progress persistence, and optional finalization; policy/audit live in `legacy/meeting/official_rounds.py` and share `legacy/meeting/turn_scheduler.py`; root HTTP module is a compatibility export | `tests/test_gui_legacy_official_round_http.py`, `tests/test_legacy_official_rounds.py`, `tests/test_gui_server_turns.py`, `tests/test_gui_server_moderation_finalization.py` |
...
| `legacy/live_agent/http/read.py` | Compatibility | Legacy resident room/return-packet reads plus diagnostic history and health/readiness projections; root HTTP module is a compatibility export | `tests/test_gui_legacy_live_agent_read_http.py`, `tests/test_gui_server_room_payload.py`, `tests/test_gui_server_session_runs.py` |
...
| `legacy/diagnostics/http/provider_health.py` | Compatibility | Provider runtime-config validation and optional bounded probe dispatch with public-safe diagnostic projection; report policy lives in `diagnostics/provider_health.py`; root HTTP module is a compatibility export | `tests/test_gui_legacy_provider_health_http.py`, `tests/test_gui_server_streams_http.py`, `tests/test_provider_health.py` |
...
| `legacy/live_agent/http/session.py` | Compatibility | Legacy resident-session mutations backed by `legacy/live_agent/session_service.py`; root HTTP module is a compatibility export | `tests/test_gui_server_session_lifecycle.py`, `tests/test_gui_server_session_recovery.py`, `tests/test_live_agent_session_agent_controls.py`, `tests/test_legacy_live_agent_session_service.py` |
| `legacy/live_agent/http/process.py` | Compatibility | Legacy process-group mutations backed by `legacy/live_agent/process_service.py`; root HTTP module is a compatibility export | `tests/test_gui_server_process_smoke.py`, `tests/test_legacy_live_agent_process_service.py` |
| `legacy/live_agent/http/session_run.py` | Compatibility | Durable legacy session-run controls backed by `legacy/live_agent/session_run_service.py`; root HTTP module is a compatibility export | `tests/test_gui_server_session_runs.py`, `tests/test_legacy_live_agent_session_run_service.py` |
...
| `legacy/live_agent/http/smoke.py` | Compatibility | Basic, official-round, durable session, and approval-gated real-provider smoke execution; root HTTP module is a compatibility export | `tests/test_gui_legacy_live_agent_smoke_http.py`, `tests/test_gui_server_smoke_routes.py`, `tests/test_gui_server_real_session_smoke.py` |
...
1. Completed: meeting read/lifecycle/workroom/SSE projections now live behind
   `LegacyMeetingQueryService`. `legacy/meeting/records.py` owns path
...
2. Completed: room and return-packet reads now belong to
   ...
   `diagnostics/report_projection.py`. Local CLI discovery and generated config/
...
`legacy/meeting/turn_scheduler.py`. Round/rounds/preset scheduling, progress,
