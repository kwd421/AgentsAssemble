...
- Legacy fallback routes are retired: `/legacy/` is not a supported entry point
  even when the React build exists, and legacy static assets are not packaged or
  publicly served.
...
| Legacy route | `/legacy/` | Not applicable | retired | `tests/test_gui_server.py::test_root_reports_missing_react_build_without_serving_legacy_console`, `tests/test_gui_server.py::test_root_and_app_serve_react_when_build_available` | `/legacy/` returns 404 whether or not the React build exists, so stale bookmarks cannot open a parallel room surface. |
| Static assets | `/static/*` | `/app/*`, root React assets via `/app/assets/*` | verified for React serving and legacy-static retirement, browser parity operator-verified | `tests/test_static_ui_assets.py`, `tests/test_gui_server.py::test_root_and_app_serve_react_when_build_available`, `npm run build` when React changes | React index served at `/` and `/app/` rewrites `/assets/*` to `/app/assets/*`; legacy static routes return 404 and `agentsassemble/static` is not packaged. |
...
| Discord room shell | No single vanilla equivalent | `App.tsx` with `HomeSidebar`, `MemberList`, `ChannelHeader`, and settings/context-menu components | partial | `tests/test_frontend_sidebar_resize.py`, `tests/test_static_ui_assets.py::test_react_responsive_layout_hooks_are_present`, `tests/test_static_ui_assets.py::test_react_app_surfaces_compact_room_status_without_duplicate_navigation`, `cd frontend && npm run build` | React now uses one Discord-style shell instead of a duplicated command strip: room rail, resizable channel/home sidebar, channel header, chat body, right member/details panel, and bottom user panel. The far-left room rail remains fixed; the adjacent sidebar width is clamped and persisted as local browser UI preference, not shared backend room state. Admin/release-health/resource inspection stays behind the topbar management button; the shell must not start providers, run release checks, close turns, promote chatter, or expose private session/prompt/path/credential fields. |
| Room registry | `GET/POST /api/rooms`, `/api/rooms/archive`; legacy `/api/room/ensure` compatibility | `fetchRooms()`, `createRoom()`, `archiveRoom()` | partial | `tests/test_gui_server.py::test_rooms_endpoint_lists_room_created_by_ensure`, `tests/test_gui_server.py::test_rooms_endpoint_guest_sees_only_own_rooms`, `tests/test_gui_server.py::test_rooms_archive_hides_room_from_default_list`, `tests/test_live_agent_frontend_create.py::FrontendLiveAgentCreateTests::test_ensure_frontend_meeting_writes_room_registry_owner_and_label` | React creates canonical rooms explicitly and merges server-known room metadata into the local room dock so localStorage is a cache. The legacy ensure route remains compatibility-only. Archive UI is not yet surfaced. |
...
| Discord home friends | No vanilla equivalent | `FriendsView`, `FriendProfileCard`, `FriendDmPanel`, and `HomeSidebar` using `fetchRoomFriends()`, `addRoomFriend()`, `fetchRoomFriendDm()`, `postRoomFriendDm()`, `createLiveAgentJoinBrief()`, `/api/room-friends/dm`, `/api/live-agents/<agent_id>/dm-reply`, session resume/stop APIs, and `GET /api/room-members` | partial | `tests/test_room_friends.py`, `tests/test_gui_server.py::test_room_friends_api_saves_friends_and_suggests_live_agents`, `tests/test_gui_server.py::test_room_friend_dm_api_posts_direct_ai_messages_without_lobby`, `tests/test_live_agent_runner.py::test_runner_answers_direct_dm_without_lobby_post`, `tests/test_static_ui_assets.py::test_react_discord_home_friends_uses_persisted_room_friends`, browser check | React can persist people, subscription AI, API, Local, remote, and unknown participants as room friends, list saved friends in the Discord-home direct-message sidebar, suggest active live agents, show the selected saved friend as a Discord-style profile card with provider/type/status/recent-room evidence, display online state only from current live-agent heartbeat overlay, send direct AI DM messages for saved AI friends with a real session id, show backend resume/missing-session errors in the composer, expose RESUME and STOP(KILL) controls for the operator's own AI process groups, create a Join Brief for a saved friend, and invite a saved friend into the selected local room as a persisted member without sending Discord messages or exposing secrets. |
| User profile | No vanilla equivalent | `UserPanel` and `UserSettingsPanel` using `fetchUserProfile()` and `saveUserProfile()` | partial | `tests/test_user_profile.py`, `tests/test_gui_server.py::test_user_profile_api_saves_local_discord_identity`, `tests/test_static_ui_assets.py::test_react_user_panel_uses_persisted_discord_profile`, browser check | React persists the local Discord-style identity shown in the bottom-left user panel: display name, handle, status, custom status, avatar label, banner preset, accent color, mic mute, and deafen state. The bottom profile popover now separates visible profile/status management from account/profile/voice settings sections while staying local-only. |
...
| Side chat | `/api/side-chat` | `fetchSideChat()`, `postSideChatMessage()`, `SideChatDock` | partial | `tests/test_frontend_side_chat_runtime.py`, `tests/test_static_ui_assets.py::test_react_side_chat_uses_separate_room_contract`, `tests/test_static_ui_assets.py::test_react_side_chat_uses_shared_discord_mention_composer`, `tests/test_static_ui_assets.py::test_react_lobby_message_actions_open_side_chat_thread_context`, `cd frontend && npm run build`, browser DOM check | React surfaces unofficial side chat as a right-panel tab beside room connection information, using the same mention composer model as the main room chat so member mentions work consistently across the main channel and side panel. Lobby message rows now expose a Discord-style message action that opens the side-chat tab with the source message shown as thread context. That context is UI state over the room-scoped side-chat log, not a separate persisted Discord thread. |
| Live agents | `/api/live-agents` | `FlowResponse.agents` via `fetchLiveAgentFlow()` plus `MemberList` | partial | `tests/test_frontend_roster_truth.py`, `tests/test_frontend_agent_labels.py`, `tests/test_static_ui_assets.py::test_react_lobby_preserves_agent_owned_room_evidence`, `frontend/src/api.ts` | Host approval, context durability, join semantics, sandbox truth, character-mode badge state, owner-visible quota windows from the session-scoped flow projection, and cursor/reply evidence are rendered through the Discord-style member panel on active room surfaces. The old standalone React `Roster.tsx` side panel was removed because it had no `App.tsx` consumer; direct `/api/live-agents` parity remains vanilla-only and must not expose raw quota fields. |
...
| `/api/provider-usage/opencode` | GET | exact | `fetchProviderUsage()` | yes | Owner-visible sanitized OpenCode account usage. |
...
| `/api/rooms` | POST | exact | `createRoom()` | yes | React explicitly creates a canonical room before opening it; room creation is no longer an activation side effect. |
...
| `/api/provider-credentials/opencode` | GET | exact | `fetchProviderCredentialStatus()` | yes | Returns only configured/source metadata; never returns the credential value. |
| `/api/provider-credentials/opencode` | POST | exact | `setProviderCredential()` | yes | Stores an OpenCode key in the host secure store; the input is cleared by React after submission. |
| `/api/provider-credentials/opencode` | DELETE | exact | `deleteProviderCredential()` | yes | Deletes the OpenCode key from the host secure store; the response never returns the credential value. |
...
| `/api/server-info/challenge` | POST | exact | `-` | no | Host-key challenge for known-server verification; the trusted-client shell reads it outside the React API barrel. |
...
| `/api/room-invite/agent-join` | POST | exact |  | no | Agent Bridge invites are consumed only by the native attendee path after signed client/provider scope validation; browser preflight rejects them without mutation. The Python attendee client is not a React API wrapper. |
...
- `/legacy/` is retired and returns 404 even when a React build exists, so
  stale bookmarks cannot reach a second app.
- `/static/*`, `/legacy/static/*`, and the old `agentsassemble/static`
  package data are removed from the current frontend surface.
