...
| Discord home friends | No vanilla equivalent | `FriendsView`, `FriendProfileCard`, `FriendDmPanel`, and `HomeSidebar` using `fetchRoomFriends()`, `addRoomFriend()`, `fetchRoomFriendDm()`, `postRoomFriendDm()`, `createLiveAgentJoinBrief()`, `/api/room-friends/dm`, and `/api/room-members` | partial | `tests/test_room_friends.py`, `tests/test_gui_server.py::test_room_friends_api_saves_friends_and_suggests_live_agents`, `tests/test_gui_server.py::test_room_friend_dm_api_posts_local_messages_only_for_saved_friends`, `tests/test_static_ui_assets.py::test_react_discord_home_friends_uses_persisted_room_friends`, browser check | React can persist people, subscription AI, API, Local, remote, and unknown participants as room friends, list saved friends in the Discord-home direct-message sidebar, suggest active live agents, show the selected saved friend as a local Discord-style profile card with provider/type/status/recent-room evidence, store local DM messages for saved friends only, create a Join Brief for a saved friend, and invite a saved friend into the selected local room as a persisted member without starting providers, sending Discord messages, or exposing secrets. |
