...
| Room registry | `/api/rooms`, `/api/room/ensure`, `/api/rooms/archive` | `fetchRooms()`, `ensureRoomMeeting()`, `archiveRoom()` | partial | `tests/test_gui_server.py::test_rooms_endpoint_lists_room_created_by_ensure`, `tests/test_gui_server.py::test_rooms_endpoint_guest_sees_only_own_rooms`, `tests/test_gui_server.py::test_rooms_archive_hides_room_from_default_list`, `tests/test_live_agent_frontend_create.py::FrontendLiveAgentCreateTests::test_ensure_frontend_meeting_writes_room_registry_owner_and_label` | React auto-merges server-known room metadata into the local room dock so localStorage is a cache. The DB stores only room registry/ownership metadata, not lobby or message bodies. Archive UI is not yet surfaced. |
...
| `/api/room/ensure` | POST | exact | `ensureRoomMeeting()` | yes | React promotes the active local room to a server-backed meeting and room registry row on demand. |
