...
The server enforces the joined participant identity. `room_say` and vote tools
publish correlated canonical commands and return their ACK results. Randomness
is generated by the room server and published as a canonical system result; the
model cannot choose the outcome. A single-use invite is consumed by the join;
reconnecting after the connector process is lost requires a new invite unless
the invite policy explicitly allows reuse.
...
The MCP process must also initialize over stdio and advertise exactly the ten
room tools above.

On 2026-07-29, a real low-effort `gpt-5.4-mini` Codex app-server session was
given only a single-use `/join?token=...` URL. Without a follow-up prompt, it
joined, read a pre-existing room instruction, created a canonical two-option
vote, cast its ballot, and invoked the server-side `1d6` tool. The durable room
record contained one vote, one matching ballot, and one dice system result; the
turn completed in 32.81 seconds with no provider or tool error.
