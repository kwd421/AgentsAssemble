...
| Main-message edit and delete | `views/lobby/MessageMutationControls.tsx`, `views/LobbyView.tsx`; server owner `room/messages.py` and room repositories | In a disposable Safari room, an ordinary message was edited in place and showed `(수정됨)`; search returned the new text. The centered delete dialog showed author, exact time, content preview, and a red action. Deleting produced `삭제된 메시지입니다`, removed the body from search and the pin list, and the tombstone remained after browser reload. Repository and realtime tests cover rollback, unauthorized edit rejection, attachment removal, idempotent retry, and no second `message_final` record | Live verified | Extend the same product contract to custom-channel and vote messages; repeat owner/Agent-Session-owner/guest moderation in a multi-client fixture |
