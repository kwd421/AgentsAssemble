...
| App command menu and vote entry | `views/components/ComposerCommandMenu.tsx`, `VoteComposerDialog.tsx`, `VotePollCard.tsx`; server summary owner `room/votes.py` | In disposable Safari rooms, the app control opened `/vote`, created a two-option poll, and accepted a ballot. The card showed only anonymous totals and `내 선택`; no voter-attributed activity row appeared, and the same aggregate/own-choice state returned after browser reload. Browser projection, provider-private mirrors, Python/MCP summaries, and the terminal helper now expose only aggregate totals plus that viewer/Agent Session's own choice; persisted provider files contain no ballot rows | Live verified | Complete a live close/revote smoke with multiple participants, then repeat independent vote inspection with each supported real provider |
