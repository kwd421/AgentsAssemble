...
| App command menu and vote entry | `views/components/ComposerCommandMenu.tsx`, `VoteComposerDialog.tsx` | In a disposable Safari room, the app control opened `/vote`, created a two-option poll, accepted one ballot, and rendered the updated total and ballot activity | Live verified | Complete a live close/revote smoke with multiple participants |
