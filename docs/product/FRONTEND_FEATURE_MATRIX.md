...
| App command menu and vote entry | `views/components/ComposerCommandMenu.tsx`, `VoteComposerDialog.tsx`, `VotePollCard.tsx`; server summary owner `room/votes.py` | In disposable Safari rooms, the app control opened `/vote`, created a two-option poll, and accepted a ballot. The card showed only anonymous totals and `내 선택`; no voter-attributed activity row appeared. Selecting that option again withdrew the ballot, returned both options and participant count to zero, and remained withdrawn after browser reload and room re-entry. A live creator-close smoke showed the explicit close control, immediate `마감됨` state, disabled options, removed close control, and the same closed state after browser reload and room re-entry. A separate live smoke created a vote with `마감 시간 없음`, disabled the minute field while selected, persisted the zero-duration vote, and rendered the same no-deadline label after reload. Manual close is a canonical state event, deadline close is derived automatically, and both reject later ballots. Browser projection, provider-private mirrors, Python/MCP summaries, and the terminal helper expose only aggregate totals plus that viewer/Agent Session's own choice; persisted provider files contain no ballot rows. Internal harnesses and the external Room Connector expose explicit withdrawal and creator-close actions | Live verified | Complete a live close/revote smoke with multiple participants, then repeat independent vote inspection with each supported real provider |
...
| Channel search | `views/components/ChannelHeader.tsx`, `views/LobbyView.tsx`, `views/CustomChannelView.tsx`; server owner `features/message_search/`; Agent Session boundary `providers/room_portal_search.py` | The derived SQLite FTS index found a message older than 260 later records with the whitespace-insensitive query `배포 오류`; Safari kept results open, loaded only the bounded 16-event context, highlighted the original, hid misleading beginning/unread state, and returned to messages 230–259 through `최근으로 이동하기`. A second two-channel Safari fixture returned `#general` and `#release-notes` together and preserved query, scope, results, channel switch, and original-message highlighting in both directions. Automated coverage proves newest-first 30-item paging without a total cutoff, attachment-name indexing, exclusion of non-message activity, the shared API/MCP/terminal-helper provider path, external connector access, and bridge-only session-token ownership | Live verified | Repeat the structured search flow with each supported real provider and in a narrow browser layout |
...
| Pinned messages | `views/components/ChannelHeader.tsx`, `views/LobbyView.tsx`, `views/CustomChannelView.tsx`; server owner `features/message_pins/routes.py` and room repositories | In a live Safari room, a keyboard-accessible message action pinned a real message, the channel list selected and focused its original, browser reload retained it, and unpin removed it immediately. Automated authorization checks reject read-only humans and agents while allowing them to inspect the list | Live verified | Repeat original-message navigation for a pin older than the initially loaded history and across two human browser participants |
| Main-message edit and delete | `views/lobby/MessageMutationControls.tsx`, `views/LobbyView.tsx`; server owner `room/messages.py` and room repositories | In disposable Safari rooms, an ordinary message was edited in place and showed `(수정됨)`; search returned the new text. The centered delete dialog showed author, exact time, and content preview. Ordinary deletion removed the body from search and pins. Vote deletion showed the poll question, removed the poll card, ballot activity, totals, and ballot identity together, and retained exactly one `삭제된 메시지입니다` tombstone after browser reload. Repository and realtime tests cover rollback, authorization, attachment removal, idempotent retry, and no provider wake | Live verified | Extend the same product contract to custom-channel messages; repeat owner/Agent-Session-owner/guest moderation in a multi-client fixture |
