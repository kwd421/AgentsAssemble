...
| Friends, direct-message and local-profile HTTP | saved-friend records and live-agent projection in `features/social/friends.py` with root compatibility export in `room_friends.py`; routes in `features/social/routes.py` with root compatibility export in `gui_social_http.py`; direct-message process callback wired in `gui.py` |
