...
| Authorized room message search | derived SQLite FTS indexing, paging, and bounded context reads in `features/message_search/`; public message normalization in `features/jsonl_chat.py`; browser query state in `frontend/src/views/useRoomMessageSearch.ts` and channel timeline views |
