...
| Legacy resident session mutations | group/session lifecycle in `legacy/live_agent/session_service.py`; durable run lifecycle in `legacy/live_agent/session_run_service.py`; session-run HTTP registrar in `legacy/live_agent/http/session_run.py`; root compatibility exports remain; session lifecycle HTTP remains `gui_legacy_live_agent_session_http.py` pending its move |
...
| Legacy resident roster and admission projections | query facade and admission evidence in `legacy/live_agent/roster_queries.py` with root compatibility export; HTTP in `gui_legacy_live_agent_read_http.py` |
...
| Legacy resident preflight | service in `legacy/live_agent/preflight.py`; safe response projection in `diagnostic_report_projection.py`; HTTP registrar in `legacy/live_agent/http/preflight.py`; service and HTTP retain root compatibility exports |
