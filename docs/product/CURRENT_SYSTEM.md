...
| Legacy resident health aggregation | shared safe status and reason policy in `legacy/live_agent/health.py` with root compatibility export; aggregation in `legacy_live_agent_health_queries.py`; observation cursor/event policy in `legacy_live_agent_observation_health.py`; durable run/monitor policy in `legacy_live_agent_session_run_health.py` |
