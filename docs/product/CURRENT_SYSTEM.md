...
| Legacy resident health aggregation | shared safe status and reason policy in `legacy/live_agent/health.py`; observation cursor/event policy in `legacy/live_agent/observation_health.py`; both retain root compatibility exports; aggregation in `legacy_live_agent_health_queries.py`; durable run/monitor policy in `legacy_live_agent_session_run_health.py` |
