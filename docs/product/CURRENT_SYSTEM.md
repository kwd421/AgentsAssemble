...
| Invite/session persistence | contracts and fail-closed default in `admission/repository.py`; durable workflow allowlist in `admission/workflow_record.py`; local memory/JSON adapters temporarily remain in `room_invite_repository.py`; hosted owner in `persistence/postgres/admission/`; compatibility export in `postgres_invite_repository.py`; selection in `room_invite_repository_factory.py` |
