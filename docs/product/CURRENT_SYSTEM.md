...
| GUI HTTP routing, response, static delivery, and WebSocket transport | route/request-context owner in `web/router.py`; response owner in `web/response.py`; static owner in `web/static.py`; shared SSE/WebSocket cadence in `web/sse_cadence.py`; ticket and WebSocket upgrade owner in `web/websocket.py`; root compatibility exports retained; composition in `gui.py` |
| GUI Host/Origin and public-route trust policy | owner in `web/security.py`; compatibility exports in `gui_request_security.py` |
| Stable public invite entrypoint announcement | repository-relative configuration and asynchronous Cloudflare KV announcement in `application/stable_entry.py`; root compatibility export retained; tunnel lifecycle in `public_tunnel.py` |
