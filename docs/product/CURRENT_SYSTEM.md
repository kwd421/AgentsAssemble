...
| Legacy resident health aggregation | shared safe status and reason policy in `legacy/live_agent/health.py`; observation cursor/event policy in `legacy/live_agent/observation_health.py`; durable run/monitor policy in `legacy/live_agent/session_run_health.py`; each retains a root compatibility export; aggregation in `legacy_live_agent_health_queries.py` |
