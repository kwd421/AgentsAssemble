...
| Friends, direct-message and local-profile HTTP | saved-friend records and live-agent projection in `features/social/friends.py`; friend-DM JSONL and live-agent delivery in `features/social/direct_messages.py`; root compatibility exports in `room_friends.py` and `room_friend_dms.py`; routes in `features/social/routes.py` with root compatibility export in `gui_social_http.py`; direct-message process callback wired in `gui.py` |
