...
| Routing and provider context | `room_routing.py`; bounded room projection in `room/context.py` with compatibility export in `room_context.py`; turn packet assembly in `room_turn_context.py`; provider delivery cursor parity in `providers/sync_cursor.py` with compatibility export in `room_provider_sync_cursor.py` |
