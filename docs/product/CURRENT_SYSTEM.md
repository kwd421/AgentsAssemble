...
| Identity, credential, membership compatibility, preference, and usage persistence | storage-independent contract and normalization in `identity/repository.py`; local SQLite implementation and registry remain temporarily in `identity_store.py`; hosted owner in `persistence/postgres/identity/`; compatibility exports in `postgres_identity_*.py`; selection in `identity_repository_factory.py` |
