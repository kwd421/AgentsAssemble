...
- resident Agent Bridge turns assigned over the canonical room WebSocket
- Codex app-server Agent Session runtime with bounded provider-visible turn input
...
JSON room packet remains available for dry-run/debug and recovery seed paths.

The retired direct turn endpoints return HTTP 410 instead of launching a
one-shot provider command. Resident providers reuse their existing runtime.
