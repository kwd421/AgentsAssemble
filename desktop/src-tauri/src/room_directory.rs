...
fn cache_path(app: &AppHandle) -> Result<PathBuf, String> {
    app.path()
        ...
        .map_err(|error| format!("cannot locate desktop room cache: {error}"))
}
...
pub fn sanitize_directory(payload: &str) -> Result<String, String> {
    if payload.len() > MAX_PAYLOAD_BYTES {
        return Err("desktop room cache is too large".to_owned());
    }
    let parsed: Value = serde_json::from_str(payload)
        .map_err(|error| format!("desktop room cache is invalid JSON: {error}"))?;
    let rooms = parsed
        .as_array()
        .ok_or_else(|| "desktop room cache must be an array".to_owned())?;
    let sanitized = rooms
        .iter()
