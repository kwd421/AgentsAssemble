...
test("host-signed endpoint leases are monotonic and immediately revocable", async () => {
  ...
  const stale = await signedHostRequest(env, serverId, host.pair, "PUT", {
    ...endpointBody,
    generation: 99,
  });
  ...
  const offline = await signedHostRequest(
    env,
    serverId,
    host.pair,
    "DELETE",
  ...
  assert.equal(
    (await payload(afterStop)).servers[0].endpoint.status,
    "offline"
  );
});
...
test("CORS reflects only approved loopback/quick-tunnel origins", async () => {
  ...
  const allowed = await worker.fetch(
    new Request("https://central.example/healthz", {
      headers: { origin: "https://safe-name.trycloudflare.com" },
    }),
    env,
    {}
  );
